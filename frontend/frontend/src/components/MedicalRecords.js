...
const MedicalRecords = () => {
  ...
  const fetchRecords = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/medical-records/patient', {  // Explicit URL with port 5000
  ...
  const handleAddRecord = async (e) => {
    ...
    try {
      await axios.post('/api/medical-records/add', formData, {  // Explicit URL with port 5000
        headers: {
